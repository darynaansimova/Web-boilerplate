...
function getUsers(apiUsers) {
  const rawUsers = [...apiUsers].map(normalizeUser);
...
class TeacherManager {
  ...
  applyFilters({ resetPage = true } = {}) {
    console.log('Applying filters, resetPage = ', resetPage);
    ...
    if (this.currentSearchQuery) {
      this.applySearchWithFilters(this.currentSearchQuery, { resetPage });
    } else {
  ...
  applySearchWithFilters(searchQuery, { resetPage = true } = {}) {
...
setupTableSorting() {
  ...
  searchInput.addEventListener('input', () => {
    ...
    debounceTimeout = setTimeout(() => {
      ...
      if (query === '') {
        ...
        this.applySearchWithFilters(query, { resetPage: true } );
      }
    }, 250);
  });
}
  ...
  performSearch() {
    ...
    this.applySearchWithFilters(searchQuery, { resetPage: true });
  }
  ...
  //LAB 4 - Task 1: Load data from API
